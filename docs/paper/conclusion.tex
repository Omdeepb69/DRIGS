...
Empirical evaluations on dual NVIDIA Tesla T4 GPU nodes demonstrate that DRIGS's \texttt{PriorityScheduler} reduces P95 queue wait time by \textbf{60.09\%} ($2.740$\,s vs.\ FIFO $6.866$\,s) under uniform inter-arrival intervals ($\mathcal{U}(0.5,2.0)$\,s, $N=15$ jobs/trial, 5 independent trials). For multi-GPU workloads, \texttt{TopologyAwareScheduler} achieves a \textbf{22.19\%} higher topology-quality score ($74.89$ vs.\ $61.29$) than random placement. In fault recovery benchmarks, DRIGS executes pure control-plane rescheduling in \textbf{0.496\,ms} and achieves complete physical end-to-end wall-clock recovery following process \texttt{SIGKILL} termination in \textbf{129.13\,ms}. At high scale ($N=1,000$ jobs queue depth), DRIGS maintains scheduler decision latencies under \textbf{17\,ms} and SQLite control-plane enqueue throughput of $81.7$\,jobs/sec. Under 95\% VRAM saturation, DRIGS gracefully throttles job completion rate to 40.0\%, preventing GPU memory over-commitment without incurring a CUDA OOM exception, and recovers synthetic OOM injection within \textbf{0.021\,ms}.
...
\begin{enumerate}
    ...
    \item \textbf{Synthetic workload distributions.} Experiment A uses synthetically generated job traces with randomly sampled workload templates, GPU counts, VRAM requests, and durations. Scheduler behavior under real production AI workload distributions (e.g., published cluster traces) is not evaluated.
    \item \textbf{Single GPU model.} Physical evaluation uses a single GPU model (NVIDIA Tesla T4). Scheduling behavior on mixed-GPU clusters (e.g., T4 + A100) or NVLink-connected GPU topologies is not empirically characterized; such evaluation is left as future work.
\end{enumerate}

\subsection*{Reproducibility \& Artifact Availability}
Source code, benchmark runner scripts (\texttt{experiments/}), seeded workload generators, and raw experimental outputs (JSON) are available at \url{https://github.com/Omdeepb69/DRIGS}. Hardware configuration: 2$\times$ NVIDIA Tesla T4 (15\,GB GDDR6 VRAM each), PCIe Gen3 x16 (PHB tier), CUDA 12.2, Linux kernel 5.15. Software stack: Python 3.10+, PyTorch 2.x, NCCL 2.x. Experiments are fully reproducible via \texttt{python experiments/harness.py} with seeds documented in Section~\ref{sec:evaluation}. Scheduling policy experiments use base seed 42 (trial $i$: seed $42 + 1000i$), covering all random draws including inter-arrival times, job durations, GPU counts, VRAM requests, and base priorities.
